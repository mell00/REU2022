...
# Edited out reference to FitAR package and FitAR function
# Replaced with ar function from package stats

shortbaar = function(k, time, data, iterations, burn_in = 50, make_murder_p = 0.5, percent = 0.02, lambda = 1, jump_p = 0.25, ar = 1, progress = TRUE, fit_storage = FALSE){
  ...
  fitMetrics<-function(k_ends, full_data){
    ...
    if(length(k_ends) < 3 ){
      ...
      model = suppressWarnings(ar(full_data[,2],aic=F,order.max=ar,demean=T))
      #      model = suppressWarnings(arima(y_values,order=c(ar,0,0)))
      ...
      for(i in 2:length(k_ends)) {
        if(i == 2){
          ...
          model = suppressWarnings(ar(y_values,aic=F,order.max=ar,demean=T)) 
          #       model = suppressWarnings(arima(y_values,order=c(ar,0,0)))
          sub_n = length(y_values)
          SEE = sum(na.omit(model$res)^2)
          s2 = SEE/sub_n
          sub_loglik = (-1*sub_n/2)*(log(2*pi)+log(s2)+1)
          #       sub_loglik = model$loglik
