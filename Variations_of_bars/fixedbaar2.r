...
#Sets up function, parameters and requirements
baar = function(k, time, data, iterations, burn_in = 50, make_murder_p = 0.5, percent = 0.02, lambda = 1, jump_p = 0.25, ar = 1, progress = TRUE, fit_storage = TRUE){
  ...
  ar = floor(ar)#Rounds down to nearest integer for model
  ...
  freeObservations<-function(k_ends, ar){
    ...
    if(ar == 1){
      ...
      constraint = (2*ar-1) #Determine space between breakpoints
    }
    ...
    for(a in 1:length(k_ends)){
      if(a == 1){
        ...
        left_set = full_set[left:k_ends[[a]]] #makes new interval of unavailabele spaces
  ...
  barJiggle<-function(percent, k_ends){
    ...
    if(ar == 1){ #I believe this code jiggles subsequent breakpointsin order to fulfill space constraint
      constraint = (3 - 1)
    }else{
      constraint = (2*ar-1)
    }
    ...
    if(length(final_neighborhood) > 1){
      ...
      return("jiggle failure") #informs user if jiggle criteria is not met
    }
  ...
  newEnds<-function(k_ends, make_k, murder_k){
    ...
    if(max(diff(k_ends)) >= constraint & length(k_ends) < 3 | max(diff(k_ends)) >= constraint & u_step <= make_k){
      type = "add" #think that sets up add as a command prompt
      a.count <<- a.count + 1 #think it counts how many times we add
      ...
      type = "sub" # think that sets up sub as a command prompt
      s.count <<- s.count + 1 #think it counts how many times we sub
