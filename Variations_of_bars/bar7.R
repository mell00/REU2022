...
bar7 = function(k, time, data, iterations, make_murder_p, percent){
  ...
  for(i in 1:iterations){
    ...
    if(length(k_ends) < 3 | u_step <= make_k){
      ...
      n_unfree = setdiff(c(1:n), sort(n_free))
      
      q2 = make_k * part_two_q_add_score_add(c(1:n), n_unfree, k_ends, k_ends_new, make_k)
...
#current_result = bar7(c(30,60), test_data_2[,1], test_data_2[,2], 200, 0.6, 0.03)
