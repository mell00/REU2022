    ...
    if(length(k_ends) < 3 ){
      ...
      for(i in 2:length(k_ends)) {
        if(i == 2){
          ...
          model = FitAR(y_values, p=1) #running a AR on the selected interval 
          sum_loglik = sum_loglik + model$loglikelihood #the logLik looks the log likelyhood (relates to both SSR and MLE)
        }else if(i > 2){
          min = k_ends[i-1]
